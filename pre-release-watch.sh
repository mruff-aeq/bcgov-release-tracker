...
# API: every first-parent commit on the default branch after the commit on test
# is one merged PR (squash or merge commit), so `git log STOP..origin/HEAD` is
# the exact, complete list — no 100-PR API window that can miss the bounding
# commit, no sort-by-updated staleness. Columns are derived from the commit:
# PR number and title from the subject, ticket from a bcgov/entity#N ref or the
# leading number of the title ("34267 - ..."), author from the GitHub noreply
# email (login) or the git author name.
#
...
if [ "$TEST_RELEASE" -eq 1 ]; then
  ...
  # branch AFTER the commit on test is one merged PR not yet on test (see
  # ensure_clone / list_prs). --in-dirs is a pathspec on that log.
  ensure_clone || exit 1
  if ! git -C "$CLONE_DIR" cat-file -e "${STOP_SHA}^{commit}" 2>/dev/null; then
    echo "test-release: commit $STOP_SHA (newest test run of $WORKFLOW) is not in $REPO's history — cannot bound the PR list." >&2
    [ "$HTML" -eq 1 ] && echo "warning: test commit $STOP_SHA not found in $REPO history — no PR list"
    exit 0
  fi
  if ! git -C "$CLONE_DIR" merge-base --is-ancestor "$STOP_SHA" origin/HEAD 2>/dev/null; then
    # Deployed from a feature/hotfix branch: the range below then lists every
    # default-branch commit not reachable from it, which may overshoot.
    echo "test-release: warning — test commit $STOP_SHA is not on $REPO's default branch; the list may be inexact." >&2
    [ "$HTML" -eq 1 ] && echo "warning: test commit $STOP_SHA is not on the default branch — list may be inexact"
  fi

  list_prs "$STOP_SHA..origin/HEAD"
